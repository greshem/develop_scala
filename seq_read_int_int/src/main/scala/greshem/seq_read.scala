...
object seq_read {
  def main(args: Array[String]): Unit = 
  {
    ...
    val data = ctx.sequenceFile(inputFile, classOf[IntWritable], classOf[IntWritable]).map {  each => (each._1.get(), each._2.get()) } 
    //val data = ctx.sequenceFile(inputFile, classOf[IntWritable], classOf[IntWritable]).map {  each=>(each._1.getBytes))} 
